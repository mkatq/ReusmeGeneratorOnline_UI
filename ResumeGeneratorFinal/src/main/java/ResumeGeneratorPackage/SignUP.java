...
public class SignUP extends JFrame{
    ...
    JLabel NumberM1 = new JLabel("Your phone number must  start with 05 and contain 10 numbers");
   ...
   // public static  String Username = "";
    ...
    static boolean isNumber(String s)
    {
     ...
     static boolean isValidPhone(String s)
     {
           ...
           return s.charAt(0) ==  '0' && s.charAt(1) == '5' && s.length() == 10 && s.matches("[0-9]+");
     }
...
bt.addActionListener(
    ...
    if(isNumber(FirstName) && isNumber(MiddleName)&& isNumber(LastName)
               && isValidEmail(Email) 
                       && passf.getText().equals(ConfPassWordField.getText()) && isValidPhone(PNumber) )
   ...
   try 
    ...
    String sql2= String.format("INSERT INTO userinformation(FirstName,MiddleName,LastName,Email"
            + " , PhoneNumber,UserName,PassWord)\n"+
            "Values ('%s','%s','%s','%s','%s','%s','%s');",
            FirstName,MiddleName,LastName,Email,PNumber ,Username, PassWord);
